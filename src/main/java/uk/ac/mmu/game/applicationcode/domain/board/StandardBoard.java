...
public class StandardBoard implements Board {
    ...
    public void move(Player player, int diceRoll) {
        ...
        if (outerPos != -1) {
            ...
            if (diceRoll <= distanceToFinal) {
                ...
                if (tailIndex >= tailLength()) tailIndex = tailLength() - 1;
            ...
            if (newTailPos >= tailLength()) newTailPos = tailLength() - 1;
    ...
    public void setPos(Player player, String pos) {
        ...
        if (outer != -1) outerBoard[outer].remove(player.getName());

        int tail = findPlayerOnTail(player);
        if (tail != -1) innerBoards[player.getIndex()][tail] = null;
